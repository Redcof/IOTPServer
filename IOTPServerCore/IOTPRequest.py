...
class IOTPRequest:
    ...
    def fn_master_request_parser_iotp_uci(request_uci):
        res = regex.match(
            r"^iotp://((?P<signin>sign/(?P<username>[^/]{6,15})/(?P<password>[^/]{6,15}))|((tok/(?P<token>[0-9a-f]{32}))/(?P<slv_id>s[0-9]+)((?P<slv_qer>\?)|(/(((?P<anlg_id>a[1-7])?((?P<anlg_qer>\?)|(/(?P<anlg_val>[0-9]{1,4}))))|((?P<digt_id>d[1-7])?((?P<digt_qer>\?)|(/(?P<digt_st>[01])))))))))$",
